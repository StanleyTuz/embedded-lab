...
(2^16 counts) * prescaler [cycles/count] / (16M cycles/sec) >= 500 ms.
Prescaler=256 works.

With this prescaler, each count is 256 cycles, so each count is
(256 cycles/count) / (16M cycles / sec) = 16 us / count.

Thus for 500 ms, we need
500 ms / (16 us/ count) = 31,250 counts
So we will do a compare match interrupt at 31,249.
...
void setup_Timer1() {
  ...
  OCR1A = 31249;  // When it reaches 31,250-1 trigger interrupt
...
{
  // Toggle the LED
  // digitalWrite(13, !digitalRead(13));  // use Arduino API
  PINB = (1 << PB7);  // use registers directly
}